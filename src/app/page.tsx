...
export default function Home() {
  ...
  return (
    <div className={styles.root}>
      ...
      {/* Global Signal Map — positioned below Console and above Footer */}
      <div className={styles.dotMapWrap} aria-hidden="true">
        <svg viewBox="0 0 840 240" className={styles.dotMapSvg}>
          {DOTS.map(([cx, cy], i) => (
            <circle key={i} cx={cx * 10 + 5} cy={cy * 10 + 5} r="1.4" fill="rgba(255,255,255,0.12)" />
          ))}
          {HIGHLIGHT_DOTS.map(([cx, cy], i) => (
            <circle key={`h${i}`} cx={cx * 10 + 5} cy={cy * 10 + 5} r="2.4" fill="#ea3a00" opacity="0.8" />
          ))}
        </svg>
      </div>
      ...
      <footer className={styles.footer}>
        <div className={styles.footerTop}>
          <span className={styles.footerLogo}>Æ</span>
